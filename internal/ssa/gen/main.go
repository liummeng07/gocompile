...
func genOp() {
	...
	fmt.Fprintln(w, "\"liummeng07/gocompile/obj\"")
	for _, a := range archs {
		if a.pkg != "" {
			fmt.Fprintf(w, "%q\n", a.pkg)
		}
	...
	for _, a := range archs {
		...
		for i, r := range a.regnames {
			num[r] = int8(i)
			pkg := a.pkg[len("liummeng07/gocompile/obj/"):]
			var objname string // name in liummeng07/gocompile/obj/$ARCH
			switch r {
			case "SB":
				// SB isn't a real register.  liummeng07/gocompile/obj expects 0 in this case.
				objname = "0"
			case "SP":
				objname = pkg + ".REGSP"
			case "g":
